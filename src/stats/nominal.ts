// 公称プラン (ソルバー基盤モジュール8)
//
// 「中央値決め打ち」の乱数(NominalRng)でエキスパートポリシーを終局まで回し、乱数依存性の低い
// 目安の行動列(公称プラン)を作る。乱数依存特技(みだれぬい・ぬいパワーシフト・無我の境地)
// は2手目以降の選択から除外する(結果が乱数の偏りに左右されやすいため)。

import type { Action, GameState, Rng } from '../core';
import { pickExpert } from './policy';
import type { NominalPlan, PlanStep, SolverContext } from './types';
...
/** みだれぬい・ぬいパワーシフト・無我の境地(乱数依存の大きい特技)の除外IDセットを作る。 */
function nominalExcludeIds(ctx: SolverContext): Set<string> {
  return new Set(
    ctx.engine
      .listSkills()
...
/**
 * 公称プラン: firstAction を適用後、乱数依存特技を除いたエキスパートポリシーを NominalRng で
...
export function nominalPlan(ctx: SolverContext, state: GameState, firstAction: Action): NominalPlan {
  ...
  let current = state;
  const exclude = nominalExcludeIds(ctx);
  ...
  while (!current.finished && iterations < MAX_STEPS) {
    ...
    const picked = pickExpert(ctx, current, { exclude });
