...
- ソースは別プロジェクト **`E:\dev\dq10-saihou-landing`**(このリポジトリ外・別リポジトリ)。詳細・デプロイ手順は同フォルダの `README.md`。
...
- **公開前の差し替え必須**: ①サイト名/ブランド(現在は仮名) ②デモアプリの公開URL(現在プレースホルダ `https://dq10-saihou-sim.pages.dev/`)。**公開はブランド確定後**に行う。
