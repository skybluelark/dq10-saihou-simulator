...
| `.github/workflows/deploy.yml` | skybluelark でのみ動作(`repository_owner=='skybluelark'`)。従来の GitHub Pages デプロイ(当面維持。**アプリのみ・サブパスなし**) |
...
- CTA/リンクは末尾スラッシュ付き(`/sample/`)で参照する(相対解決を正しくするため)。
- GitHub Pages(skybluelark)側は従来どおり**アプリのみ**を配信(サブパス無し)。サブパス構成は fluonote のみ。
